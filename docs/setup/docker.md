# ProperDocs in Docker

There are multiple reasons, why you may want to use ProperDocs in docker instead of natively:

- You do not have Python installed on your system.
- You want the extra security of not having to execute random Python code (ProperDocs plugins and their dependencies) on your host system.
- You have many ProperDocs sites using almost all the exactly same plugins and it is a pain to update the dependencies regularly in all your Python virtual environments.
...
COPY default-properdocs.yml /share/properdocs.yml
WORKDIR /share
ENTRYPOINT ["/usr/local/bin/properdocs"]
...
2. Installs any ProperDocs plugins which you may use in the site(s).
3. Optionally: Provides a fallback file for `properdocs.yml`.
    This is only needed of you want to mount folders without a `properdocs.yml`.
4. Executes `properdocs serve` or `properdocs build` when you start the container.

For hardening purposes you may not want to run `properdocs` or `pip install` as root inside the container.
...
COPY default-properdocs.yml /share/properdocs.yml
WORKDIR /share
ENTRYPOINT ["/home/app/.local/bin/properdocs"]
CMD ["serve", "--dev-addr", "0.0.0.0:8000"]
```
...
To serve any folder of Markdown files with the default `properdocs.yml` from the container just mount the Markdown files into the correct location in the container:

```bash
docker run --rm -it -p 8000:8000 -v "$PWD/docs:/share/docs:ro" ghcr.io/six-two/mkdocs-knowledge-base-guide
```

If you have custom settings, you also want to mount the `properdocs.yml` and maybe some other files.
